...
export class AuthService {
  ...
  constructor(private httpClient: HttpClient, private router: Router) {
    this.httpClient.get<AppUser>('http://localhost:8080/ReimbursementApi/auth/session-user', {
  ...
  login(credentials) {
    this.httpClient.post<AppUser>('http://localhost:8080/ReimbursementApi/auth/login', credentials, {
      withCredentials: true
    }).subscribe(
      data => {
        console.log('logged in');
